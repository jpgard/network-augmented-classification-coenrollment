...
DATA_FP = c("../data/nw_1970.csv", "../data/nw_1920.csv", "../data/nw_1870.csv", "../data/nw_1820.csv", "../data/nw_1770.csv",
"../data/nw_1720.csv",
"../data/nw_1670.csv",
"../data/nw_1620.csv",
"../data/nw_1570.csv",
...
#flat
flat_train_data = select(traindata, -matches("^CL_|^ML_|^BL_|^PL_"))
...
net_pl_train_data = select(traindata, matches("^PL_|CRSE_GRD_OFFCL_CD|SBJCT_CD")) %>% select(-matches("_NBR"))
...
net_pl_nn_train_data = select(traindata, matches("^PL_BIN_NBR|CRSE_GRD_OFFCL_CD|SBJCT_CD"))
...
pl_mods = build_mods(net_pl_train_data, models_by = "SBJCT_CD", model_type = "glm") 
...
pl_nn_mods = build_mods(net_pl_nn_train_data, models_by = "SBJCT_CD", model_type = "glm")
save(ml_mods, cl_mods, bl_mods, pl_mods, ml_nn_mods, cl_nn_mods, bl_nn_mods, pl_nn_mods, file = "net_mods.Rdata")
...
net_pl_test_data = select(testdata, matches("^PL_|CRSE_GRD_OFFCL_CD|SBJCT_CD")) %>% filter(SBJCT_CD != "COGSCI")
...
net_pl_nn_test_data = select(testdata, matches("^PL_BIN_NBR|CRSE_GRD_OFFCL_CD|SBJCT_CD")) %>% filter(SBJCT_CD != "COGSCI")

save(flat_test_data, net_ml_test_data, net_cl_test_data, net_bl_test_data, net_pl_test_data, net_ml_nn_test_data, net_cl_nn_test_data, net_bl_nn_test_data, net_pl_nn_test_data, file="processed_test_data.Rdata")
...
# proportion-link; (no nn)  
pl_preds_test = struct_predict(oa_df = flat_test_data, la_df = net_pl_test_data, la_nn_df = net_pl_nn_test_data, oa_mods = flat_mods, la_mods = pl_mods, la_nn_mods = pl_nn_mods, pred_type = "struct", oa_preds = test_oa_preds, class_probs = P_c) 
acc_pl_test = sum(pl_preds_test[["oa_pred_df"]]$CRSE_GRD_OFFCL_CD == pl_preds_test[["preds"]])/length(test_oa_preds)
...
# proportion-link nn
pl_nn_preds_test = struct_predict(oa_df = flat_test_data, la_df = net_pl_test_data, la_nn_df = net_pl_nn_test_data, oa_mods = flat_mods, la_mods = pl_mods, la_nn_mods = pl_nn_mods, pred_type = "struct_nn", oa_preds = test_oa_preds, class_probs = P_c) 
acc_pl_nn_test = sum(pl_nn_preds_test[["oa_pred_df"]]$CRSE_GRD_OFFCL_CD == pl_nn_preds_test[["preds"]])/length(test_oa_preds)


test_results = data.frame(t(data.frame(acc_flat_test, acc_full_test, acc_cl_test, acc_bl_test, acc_ml_test, acc_pl_test, acc_cl_nn_test, acc_bl_nn_test, acc_ml_nn_test, acc_pl_nn_test)))
...
mm_list[["pl"]] = fetch_multiclass_metrics(labs = pl_preds_test[["oa_pred_df"]]$CRSE_GRD_OFFCL_CD, preds = pl_preds_test[["preds"]])
...
mm_list[["pl_nn"]] = fetch_multiclass_metrics(labs = pl_nn_preds_test[["oa_pred_df"]]$CRSE_GRD_OFFCL_CD, preds = pl_nn_preds_test[["preds"]])
...
net_pl_blend_train_data = select(blend_traindata, matches("^PL_|CRSE_GRD_OFFCL_CD|SBJCT_CD")) %>% select(-matches("_NBR"))
...
net_pl_blend_probe_data = select(blend_probedata, matches("^PL_|CRSE_GRD_OFFCL_CD|SBJCT_CD")) %>% select(-matches("_NBR"))
...
net_pl_nn_blend_train_data = select(blend_traindata, matches("^PL_BIN_NBR|CRSE_GRD_OFFCL_CD|SBJCT_CD"))
...
net_pl_nn_blend_probe_data = select(blend_probedata, matches("^PL_BIN_NBR|CRSE_GRD_OFFCL_CD|SBJCT_CD"))
...
pl_blend_mods = build_mods(net_pl_blend_train_data, models_by = "SBJCT_CD", model_type = "glm") 
...
pl_nn_blend_mods = build_mods(net_pl_nn_blend_train_data, models_by = "SBJCT_CD", model_type = "glm")
save(ml_blend_mods, cl_blend_mods, bl_blend_mods, pl_blend_mods, ml_nn_blend_mods, cl_nn_blend_mods, bl_nn_blend_mods, pl_nn_blend_mods, file = "net_blend_mods.Rdata")
...
# proportion-link; (no nn)  
pl_probe_preds = struct_predict(oa_df = flat_blend_probe_data, la_df = net_pl_blend_probe_data, oa_mods =  flat_blend_mods, la_mods = pl_blend_mods, pred_type = "la_preds", class_probs = P_c)[["la_preds"]]
...
# # proportion link nn
pl_nn_probe_preds = struct_predict(oa_df = flat_blend_probe_data, la_df = NULL, oa_mods =  flat_blend_mods, la_mods = pl_blend_mods, la_nn_mods = pl_nn_blend_mods, la_nn_df = net_pl_nn_blend_probe_data, pred_type = "la_nn_preds", class_probs = P_c)[["la_nn_preds"]]

# build single dataframe from all probe predictions - TODO: add nn models here
probe_preds = list("flat" = flat_probe_preds, "cl" = cl_probe_preds, "bl" = bl_probe_preds, "ml" = ml_probe_preds, "pl" = pl_probe_preds, "cl_nn" = cl_nn_probe_preds, "bl_nn" = bl_nn_probe_preds, "ml_nn" = ml_nn_probe_preds, "pl_nn" = pl_nn_probe_preds)

probe_preds = lapply(probe_preds, function(x) do.call("rbind", x))
probe_preds = make_probe_df(probe_preds) 
probe_preds["CRSE_GRD_OFFCL_CD"] = flat_blend_predict[["oa_pred_df"]]$CRSE_GRD_OFFCL_CD # creates label column for probe df
save(flat_blend_predict, flat_probe_preds, bl_probe_preds, ml_probe_preds, cl_probe_preds, pl_probe_preds, cl_nn_probe_preds, ml_nn_probe_preds, bl_nn_probe_preds, pl_nn_probe_preds, probe_preds, file = "probe_preds.Rdata")
...
# proportion-link; (no nn)  
pl_test_preds = struct_predict(oa_df = flat_test_data, la_df = net_pl_test_data, oa_mods =  flat_blend_mods, la_mods = pl_blend_mods, pred_type = "la_preds", class_probs = P_c)[["la_preds"]]
# count-link nn
...
# proportion-link nn
pl_nn_test_preds = struct_predict(oa_df = flat_test_data, la_df = net_pl_test_data, oa_mods =  flat_blend_mods, la_mods = pl_blend_mods, la_nn_df = net_pl_nn_test_data, la_nn_mods = pl_nn_blend_mods, pred_type = "la_nn_preds", class_probs = P_c)[["la_nn_preds"]]


# build single dataframe from all test predictions
test_preds = list("flat" = flat_test_preds, "cl" = cl_test_preds, "bl" = bl_test_preds, "ml" = ml_test_preds, "pl" = pl_test_preds, "cl_nn" = cl_nn_test_preds, "bl_nn" = bl_nn_test_preds, "ml_nn" = ml_nn_test_preds, "pl_nn" = pl_nn_test_preds)
test_preds = lapply(test_preds, function(x) do.call("rbind", x))
test_preds = make_probe_df(test_preds)
test_preds["CRSE_GRD_OFFCL_CD"] = flat_test_predict[["oa_pred_df"]]$CRSE_GRD_OFFCL_CD # creates label column 
save(flat_test_predict, flat_test_preds, bl_test_preds, ml_test_preds, cl_test_preds, pl_test_preds, bl_nn_test_preds, ml_nn_test_preds, cl_nn_test_preds, pl_nn_test_preds, test_preds, file = "base_test_preds.Rdata")
...
test_results %<>% rbind(data.frame(model = "acc_ensemble_xgboost_test", acc = acc_ensemble_xgb_test)) %>% arrange(desc(acc))
test_results
write.csv(test_results, file = "test_results.csv", row.names = FALSE)
...
# library(ggplot2)
# # compare flat model to count-link; this is graphic used in original workshop paper (after editing)
...
# generate gpa-only model; this is probably most reasonable baseline for model performance
